...
Client.prototype.request = function(options, callback) {
  ...
  if( client.options.authMechanism ) {
	  options.headers = merge( options.headers, options.authMechanism.getHeaders() );
  }
  var buffer = '';
console.log( '------------------------------------------------------' );
console.log( options );
...
Client.prototype.postMultipart = function(postPath, data, callback) {
  var options = {
    method: 'POST',
    path: this.options.path + this.options.core + '/' + postPath,
    headers: {
...
Client.prototype.flushBuffer = function( fDone ) {
	var aDocs = this.addBuffer.slice();
	var that = this;
	this.addBuffer = [];
	this.add( aDocs, fDone );
};

Client.prototype.bufferedAdd = function( doc, fDone ) {
	this.addBuffer.push( doc );
	clearTimeout( this.addTimeout );
	if( this.addBuffer.length > this.options.maxBufferSize ) {
		this.flushBuffer( fDone );
	} else {
		var that = this;
		this.addTimeout = setTimeout( function() { that.flushBuffer( function( err ) { if( err ) console.error( err ); } ) }, this.bufferTimeout );
		fDone();
	}
};
...
Client.prototype.query = function(query, options, callback) {
  ...
  queryParams.q = query;
  queryParams.wt = "json";
  this.postMultipart("select" ,querystring.stringify(queryParams), callback);
};
